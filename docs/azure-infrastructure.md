...
> ⚠️ The `!` in the password causes Bash history expansion. Always call sqlcmd via Python `subprocess` or with quoted heredoc — never `sqlcmd -P "***SQL_PASSWORD_REDACTED***"` on a Bash command line.
...
r = subprocess.run(
    ['sqlcmd', '-S', 'tcp:crystalcares.database.windows.net,1433',
     '-d', 'CrystalCaresProd', '-U', 'crystalSQL', '-P', '***SQL_PASSWORD_REDACTED***',
     '-Q', sql, '-W', '-s', '|', '-C', '-I'],
